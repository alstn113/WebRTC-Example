...
const VideoContents = ({ roomId }: Props) => {
  ...
  const setVideoTracks = async () => {
    ...
    try {
      ...
      pcRef.current.onicecandidate = (event: RTCPeerConnectionIceEvent) => {
        if (event.candidate) {
          if (!socket) return;
          socket.emit(EVENT.SEND_ICE_CANDIDATE, {
            to: roomId,
  ...
  useEffect(() => {
    ...
    socket?.on(EVENT.NEW_USER, onCallUser);
    socket?.on(EVENT.RECEIVE_OFFER, onCallMade);
    socket?.on(EVENT.RECEIVE_ANSWER, onAnswerMade);
    socket?.on(EVENT.RECEIVE_ICE_CANDIDATE, onIceCandidateReceived);

    setVideoTracks();

    return () => {
      socket?.off(EVENT.NEW_USER, onCallUser);
      socket?.off(EVENT.RECEIVE_OFFER, onCallMade);
      socket?.off(EVENT.RECEIVE_ANSWER, onAnswerMade);
      socket?.off(EVENT.RECEIVE_ICE_CANDIDATE, onIceCandidateReceived);
